...
- fix(git): 子目录前缀改由 git 自报(rev-parse --show-prefix)——path.relative 对长短路径名/大小写敏感,CI 短路径环境悄悄算歪,现在零猜态
- feat(ai): 模型货架上线——设置里逛抱抱脸实时 GGUF 榜(直连不通自动换国内镜像,榜单内存缓存零落盘),大小/时间筛选+正倒序+模态标签,超内存的盖「你这机器带不动」灰戳,点模型展开文件清单
- feat(ai): 模型货架接上一键到位——点「下载并使用」断点续传拉 .gguf(双源+进度条+可取消),下载完自动把 AI 配置切到内置引擎并填好模型路径,不用手填
- feat(build): 安装包换向导式安装——「下一步」页面里可选安装目录(小葵验收反馈:不能选路径),卸载照旧系统一键;README 补卸载与彻底清理说明、AI 开箱文案同步货架现状
- feat(build): 应用图标上线——换掉 Electron 出厂原子图(小葵验收反馈),「你在哪里」定位标设计(节点+连线即应用关系图缩影,配色取自暗色主题);SVG 源+生成脚本入库,小尺寸走加粗简化版保任务栏 16px 不糊
- feat(ai): 联网搜索接入 Tavily 打头——设置「联网查证」下新增 Key 填写框(免费档每月 1000 次不用绑卡),Key 走认证头只存用户配置文件;源队列按小葵定序统一为 Tavily → DuckDuckGo → 维基中 → 维基英,概念/操作题分流(source 参数)随旧维基优先序一并退役,讲解/自由聊天/web_search 三条通道共用同一队列
- fix(appearance): 外观设置改了存不住、重启就回出厂——preload 存档用的是 send,主进程那边只挂 handle,一边发件一边只收 invoke,消息静默丢弃,appearance.json 一次都没写成过;改走 invoke 正常落盘,并补「IPC 收发配对」自测(哪条通道两边对不上直接报名字)
- fix(ui): 设置「Tavily 搜索 Key」区块左右不再贴边——包一层和上下行同口径的内边距容器(1.0625rem),竖排留白同步补齐
- feat(ui): Key 输入框右侧加小眼睛——点了显明文再点藏回去(默认密文),图标按钮跟「浏览…」实心按钮区分开
- feat(ai): Tavily Key 加「测一下」与清洗——设置里点一下真打官方接口,按状态码说人话(Key 不认/额度用完/被限流/连不上),200 但回话不像 Tavily 也不吹「能用」(防劫持页);保存和读档都洗净引号/Bearer 前缀/夹层空白,形状不像 tvly- 开头给黄字提醒但不拦,测一次花 1 次搜索额度
- fix(ui): 渲染层类型声明(env.d.ts)里 shared 的引用路径多了一级——skipLibCheck 一直静默吞掉,window.atlas 的 IPC 类型其实退化成了 any;路径修正,类型恢复真身
- fix(ai): 自由聊天历史窗口截错方向——从头取前几条,把刚聊完的一轮整个扔掉,窗口还常停在没被回答的旧问题上,合并逻辑顺手把它和当前问题粘成一条,小探针扭头答旧题(小葵报「回复我以前问过的问题」);改成从尾部留最近 3 对完整问答,两头收拢(开头不留半截答案、结尾不留悬空提问);自测补方向断言与「当前问题单独成条」端到端回归
- fix(ai): 质检闸管宽了——「搜一下这个文件夹是什么」被当找位置题强拦(小葵报:模型联网已答对,却被逼着重搜本地文件重来三轮,还把程序插话当用户催促);按小葵四条方向轻量化:门卫词表摘掉「搜一下/搜搜/搜出」泛搜语气词、补救提醒封顶 2→1(宁可答案差点不让用户干等)、纠错消息开头自报家门「程序自动质检,不是用户说话」并点明只补信息不用重想、判据二对账放宽到文件名级(模型爱写短路径,老口径全路径精确匹配老冤枉它;可点跳转另有命中清单卡兜底)
- fix(ai): CI 连挂 5 锤的元凶——「全网断」自测场景漏改 options 新签名,旧式传函数被当成 options 对象,fetchText 取不到回退真联网;CI 机房在美国 DDG 直连通,「查无此物xyz」真搜出一堆小说站,「全部源失败 = 空资料」断言当场炸(本地墙内 DDG 不通,全源失败歪打正着全绿,所以怎么测都对不上 CI);补回 mock 注入,自测回到确定性不碰网
- feat(ai): 「测一下」Tavily Key 改打官方 /usage 用量接口——零搜索额度成本(旧实搜探测每次花 1 额度,退役),200 即证 Key 有效,顺手报计划名和本月用量(已用/上限/还剩几次);劫持门户页/改版回话照样当「看不懂」不吹能用;解析器和文案进自测
- feat(ui): 模型货架行排版重做(参考 LM Studio)——名字做主角,下载量/更新时间缩进名字下方副行小字;文字胶囊换彩色描边圆能力章(amber 眼=看得懂图,绿音符=带语音,灰档=纯文本/向量/未知,悬停有大白话,向量「不能陪聊」挪进副行不靠悬停);行 flex 改 grid、大小/状态章/箭头定宽列行行对齐(数字右对齐等宽);「带不动」行文字全线降灰、红章保持饱和(降灰旧规则两条收拢);筛选栏与列表拉开间距;自测补 modalityKind 图标档位断言
