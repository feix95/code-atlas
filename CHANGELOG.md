...
- feat(preview): 文件预览上分色——tree-sitter 解析出关键字/字符串/注释/数字/函数名/类型名等十种角色,色号照抄 VS Code 官方 Dark+/Light+ 两套跟界面深浅色联动;零新依赖(复用体检模块的语法引擎),不认识的语言/超 100 万字老实白字不拖累预览;选中引用的行号算法同步换成 TreeWalker 版,分色的 DOM 分包不再算错行
