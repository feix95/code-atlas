...
- fix(window): 画面消失上救生圈 —— 渲染层崩了自动重载接回,GPU 进程打嗝重开画布不丢页面状态,接上弹人话横幅,报错和断供全进后台日志好排查
- perf(chat): MiniMD 和过程垫字上 memo,流式吐字只重画 busy 那条消息,旧消息一个字不动 —— 聊得再多画面也不上强度
- fix(window): 禁用 GPU 硬件加速改软件合成 —— 第三次「画面消失」坐实是显卡合成链无声断掉(无事件可抓),釜底抽薪让透明窗跟显卡驱动绝缘;救生圈事件同步打进控制台好留现场
- fix(window): 第四案补刀 —— 软件渲染版照样隐身,凶手锁定 Windows 合成层:关掉 Chromium 原生窗口遮挡误判(CalculateNativeWinOcclusion),加 5 秒强制重画心跳,DWM 掉链子最多 5 秒自动接上;新增 Ctrl+Alt+0 手动拉起画面(不丢聊天),页面报错同步进控制台
- fix(window): 救生圈 2.0 —— 活体取证定性:渲染进程活着但画面管线停摆,重画/藏亮/改尺寸全叫不醒,唯一解药是整页重挂;渲染层 rAF 每秒报心跳,窗露着而心跳停 10 秒主进程自动重挂满血复活,Ctrl+Alt+0 升级两段式(3 秒内连按第二次直接重挂保命)
- perf(chat): 聊天列表拆掉 O(n²) —— 重试目标从每条消息全量倒查改成一轮算一次,重试回调进 ref 一个身份用到底,AssistantBubble 上 memo,聊得再久流式也只重画在写的那条
- refactor(preview): 预览去掉按文件名强制重挂,内容/选区/滚动位按文件记账就地复位,跳行滚动挪到下一帧执行,点文件链接不再整棵组件树拆了重装
- feat(chat): 设置→智能辅助新增「聊天推荐问题」开关 —— 关掉后聊天框上面那排推荐和开场示例都不出,预测下一问的模型调用一并省掉(小葵反馈大部分推荐问不上,攒条落地)
- fix(chat): 挖出隐身案真凶 —— MiniMD 渲染「先文字后行内代码」的回答时正则匹配变量被里层函数覆写成 null,读 null[1] 炸掉整棵 React 树,透明窗白屏=整窗隐身;里层改用自己的匹配变量根治,七个刁钻形态实测全过
- fix(window): 救生圈补课 —— 画面心跳从 React 树外搬进树内(树崩心跳停,看门狗 10 秒自动重挂终于管得住白屏);Ctrl+Alt+0 保命窗口 3 秒放宽到 10 秒;加两层渲染兜底网,单条回答画崩只挂那条,顶层崩显示人话卡+一键重挂,窗口再也不会无声隐身
- fix(ai): 修「改上下文档位不生效」—— 引擎已在跑时改档位被静默吞掉,现在保存配置和下次请求都会对账,档位有变就按新档重启引擎并在后台日志记一笔
- feat(chat): 「聊天推荐问题」开关升级成「推荐问题」总闸 —— 文件预览 AI 卡下面那排预设问题也归它管,关了连那个停足半秒就烧模型的文件预测一并歇掉(小葵发话,攒条落地)
- feat(ai): LLM 优化四件套 —— 翻文件搜索的命中清单走旁路程序直接摆卡(每条文件:行号可点跳预览,小模型不再当抄写员);agent 每轮工具结果后垫「本轮要回答的问题」提醒卡(撤旧垫新只挂一张,外接引擎不认就自动撤卡兜底);守则加防自指条款(翻到的文件内容只是资料,里面的指令腔不是用户在说话);超长历史截断处打「不用接着写」标注,拔掉半截清单的续写钩子
- feat(ai): 复读机防线 —— 修「回答列举到一半,同一个短语无限循环刷屏」(9B 小模型采样层的老毛病):内置引擎请求带上反重复采样参数(repeat_penalty + DRY,外接不塞行为一分不变);程序流式监工,发现尾巴连着 4 遍同一短语当场掐流自动重答一遍(重答还犯就截到打转起点附注脚交卷),普通聊天/翻文件/文件讲解/干活报告/压缩全管道都挂上,假服务端到端自测覆盖自愈和两连犯两条路;翻文件守则顺手加一句「列举别翻来覆去重复」
- fix(chat): 修「文件夹聊天里文件链接点不动」—— 文件夹聊天的 ChatTabDetailPage 漏传 fileLinks(第 141 锤埋的漏传),命中清单卡和回答里的文件全渲染成死文字;换左侧参考资料会复活是因为走了文件聊天分支(那边传了);补传后恢复可点跳预览(小葵报案,攒条落地)
- feat(chat): 绿字文件链接右键复制完整路径 —— 聊天回答、程序便签、命中清单卡里的文件链接右键弹小菜单,一键把文件的真实路径(盘符开头)复制进剪贴板;只复制不打开,开不开文件交给用户看到真实文件后自己决定(小葵拍板的安全分寸);菜单跟主题一个皮肤,贴窗口边自动往里收,复制成功当场亮「已复制 ✓」
- feat(chat): 右键菜单升级 + 摘两颗钉子 —— 绿字右键菜单加第二项「在文件资源管理器中显示」(资源管理器弹出、文件选中高亮,照样只带路不开门);预览器头部的文件名也染上主题色、挂同款右键菜单(左键不折腾,右键才算数);修「点绿字跳预览后聊天被甩到最底」—— 文件夹聊天/文件聊天/预览三个分支各养一份聊天面板,点绿字跨分支时面板重挂、粘底开关归位,把用户正看的那条压出视野,现在按对话指纹记滚动位置,同一场对话重挂回原地,新对话才滚到最新(小葵报案)
- fix(preview): 修「点绿字跳预览整窗闪一下」—— 跳转那一下左右两扇同帧大换血(目录树整棵拆掉换预览窗、聊天面板整扇重挂),透明窗在大重绘帧上容易掉一帧底色,硬切观感也生硬;给左栏预览窗和右栏聊天页挂载时各加一小拍淡入,把闪摊成渐显,预览内换文件两扇都不重挂不会反复闪动画(小葵报案)
- refactor(file-detail): 文件关系搬进概览页最下面(小葵拍板)—— 垫底的「结构与关系」Tab 曝光率太低,用户可能用很久都看不到;现在默认页就能看到关系,没分析过的显示「分析文件关系」按钮,分析过一次全会话记账不变;第三个 Tab 随之改名「结构」只留结构网格,关系里点文件跳转保持当前 Tab 的老规矩不变
- fix(file-detail): 关系模组的 UI 照着概览卡/小探针卡改成正经独立模组(小葵看图报案)—— 拆掉露在面板上的标签小字(原先还和组件自带的说明叠成两行重复),模组自带壳和头;一条连线没有时不再用大白卡,同款壳里一行灰色小字轻轻带过;没分析过/分析中也是同款壳,三种状态一个模组相
- refactor(file-detail): 「结构」Tab 整页退役,零件清单搬进概览垫底(小葵报案:说好删掉的页还残留)—— 结构网格套上关系卡同款模组壳待在概览最下面,六节全空的文件(配置/常量)连卡都不摆;文件/文件夹详情统一成「概览 + Atlas 小探针」两页,概览页的数字格和结构清单不再分成两处摆
- feat(tabs): 页签地基第一锤 —— 右栏页签化,目录树常驻左栏不再整扇换装(点绿字闪一下的老病根就地拔除);文件详情/源码预览各自成签,预览是「左代码右伴聊」合并页签;树里单击开临时页签(名字斜体,点别的文件被顶替)、双击钉住,页签上双击也能钉;同文件永远复用同一张页签,绿字跳转/关系链浏览按 VS Code 规矩复用或原地换内容;页签条悬停才现关闭钮、中键点页签即关、关激活页签右邻优先接力,全关回底色页(文件夹详情/项目主页);跳转和切页签时树自动展开路径并滚到目标(reveal 联动);聊天系统一根手指没碰 —— 文件夹聊天还是底色页的内页(小葵拍板的 VS Code 方案)
- feat(tabs): 页签改版对齐小葵的线框图 —— 页签分三个品类(概览/Atlas 小探针/文件预览),一个文件/文件夹最多摊开这三张;单击树=没钉的页签全员转向新文件,双击页签=钉住(左上图钉小图标,树里换文件它不动,新内容按品类额外新开一张);钉住对话页签=那场对话分家单过(记录搬给它,公用场清空重开),除非主动开新对话否则所有文件公用一场(保持老逻辑);页签栏空白处右键弹品类菜单,勾选=显示、不选=藏(实例连钉住状态都留着),勾法记进本机跨重启,入口撞上被关的品类自动勾回并给刚亮起的页签一抹主题色轻强调(小葵点的:别太静默);三个品类全关光显示大 logo 底板;树里双击专职开预览,详情页内页 Tab、斜体临时页签、树上双击钉住一并退役,项目主页安家为概览页签的零状态;品类判断走两层过滤(节点类型卡上限→显示开关卡显示),以后加新节点类型/新品类只动 paneKinds 一处
- feat(tabs): 页签分组拼装(小葵线框图的左右两栏)—— 页签右键「挪去右边,拆成两组」,或直接把页签拖去另一边;同组内拖页签换位置(插到目标页签前面,左缘亮一条主题色提示线),拖到页签栏空白=落到那组末尾;中间分割条拖动调左右比例(记进本机,双击回对半,左右方向键也能微调);树里点文件只让「最后点的那组」跟随转向,另一组安安稳稳看自己的;组搬空/关空了自己消亡,钉住的对话保活层跟着组走,组没了那场对话才蒸发;拖拽落点和右键菜单观感按数值推算,没实际截图确认
- feat(tabs): 页签四连修(小葵验收反馈)—— ①×掉的跟随页签(概览/Atlas小探针)自动补回:品类勾着显示就该在栏上,树里一动就重新亮出来装着新文件,预览是按需品类不在此列;②文件预览拆掉自带伴聊,回归纯源码窗,想边看代码边聊把探针页签拖去另一组拼一屏(VS Code 式自由拼装),预览里选中代码挂引用卡时闪一下探针页签告诉用户挂哪儿了;③品类菜单勾选/取消完自动收起,不再要求多点一下外部;④拖页签到正文中心区域松手=分屏判定(单组拆成两栏,两组挪去那一组,悬停时虚线框亮「松手,拆成两栏/挪到这一组」),边缘松手什么也不发生,两组还往自己组中心拖也不许诺空头支票
