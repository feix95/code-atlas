...
import { clipPreview, looksBinary, planWholeFileRef, PREVIEW_MAX_BYTES, visibleLineRange } from '../src/shared/preview.ts'
...
function main(): void {
  ...
  const rng = (over: Partial<Parameters<typeof visibleLineRange>[0]>): { start: number; end: number } =>
    visibleLineRange({ scrollTop: 0, viewportHeight: 600, lineHeight: 20, totalLines: 10000, ...over })
  assert.deepEqual(rng({}), { start: 1, end: 51 }, '顶部:可视 30 行 + 前后各 20 行缓冲')
  assert.deepEqual(rng({ scrollTop: 10_000 }), { start: 481, end: 551 }, '滚到第 501 行:前带 20 行缓冲')
  assert.deepEqual(rng({ scrollTop: 200_000 }), { start: 9931, end: 10000 }, '滚出账面(防御):老实看最后一屏')
  assert.deepEqual(rng({ totalLines: 10 }), { start: 1, end: 10 }, '比一屏还短的文件:全文区间')
  assert.deepEqual(rng({ lineHeight: 0 }), { start: 1, end: 51 }, '行高没量到按 20 兜底,不许算出 NaN')
  assert.deepEqual(visibleLineRange({ scrollTop: 0, viewportHeight: 600, lineHeight: 20, totalLines: 0 }), { start: 1, end: 0 }, '空文件:空区间')
  ...
  console.log('   全文不裁 · 换行归一 · 空文件 · 谢客线量级 · 可视区账(顶部/中部/防御/短文件/行高兜底) · 二进制嗅探 · 选区几何与浮钮文案 · 整份引用的账')
}
