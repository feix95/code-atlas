...
function main(): void {
  ...
  assert.ok(
    defaultWithCustom.includes('<style_preference>') &&
      defaultWithCustom.includes('<custom_request>'),
    '风格段和自订指令都要包 XML 标签(2.0)'
  )
  ...
  for (const opt of TONE_OPTIONS) {
    ...
    if (opt.key === 'default') assert.equal(text, '', '默认档 = 不垫语气')
      ...
      assert.ok(
        text.includes('<style_preference>'),
        `语气「${opt.label}」要成段(2.0 起包 style_preference 标签)`
      )
  }
  ...
  const longCustom = buildPersonalizationPrompt(
    withPatch({ custom: '很'.repeat(CUSTOM_MAX + 300) })
  )
  ...
  const styled = withPersonalization(
    '人设在此',
    buildPersonalizationPrompt(withPatch({ tone: 'friendly', custom: '叫我小葵' }))
  )
  assert.ok(styled.startsWith('人设在此'), '人设永远在最前面')
  assert.ok(styled.indexOf('温暖') < styled.indexOf('叫我小葵'), '语气排在自订指令之前')
  assert.ok(
    styled.indexOf('<custom_request>') < styled.indexOf('以上只改你说话的方式'),
    '自订指令之后要跟收尾划线'
  )
  ...
  const deepStyled = withPersonalization(
    '人设在此',
    buildPersonalizationPrompt(withPatch({ teaching: 'deep' }))
  )
  assert.ok(
    !deepStyled.includes('<teaching_style>') && !deepStyled.includes('名词小课堂'),
    'teaching 非 brief 时风格段也不掺教学文本'
  )
  ...
  assert.deepEqual(
    sanitizePersonalization({ tone: '暴躁', warmth: 'MORE' }),
    DEFAULT_PERSONALIZATION,
    '认不出的键值回默认'
  )
  const mixed = sanitizePersonalization({
    tone: 'humorous',
    warmth: 'more',
    custom: '  x  ',
    乱七八糟: true
  ...
  for (const opt of TEACHING_OPTIONS) {
    assert.equal(
      sanitizePersonalization({ teaching: opt.key }).teaching,
      opt.key,
      `深度「${opt.key}」要认得`
    )
  }
  assert.equal(
    sanitizePersonalization({ teaching: 'deep' }).teaching,
    'deep',
    '详细档存读一个来回不丢'
  )
  ...
  for (const t of TEACHING_OPTIONS)
    assert.ok(t.label && t.hint, '每档都要有档名和介绍(参考图的两行式)')

  console.log('✅ 个性化自测全部通过')
  console.log(
    '   默认档不垫语气 · 语气三档人设 · 讲解深度三档 · 自订指令裁长 · 顺序即优先级 · 脏存档清洗'
  )
}
