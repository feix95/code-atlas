...
console.log('── 开发态(resourcesPath 没有或没货)→ 回退 node_modules ──')
{
  ...
  ok(engine.endsWith('node_modules\\web-tree-sitter\\tree-sitter.wasm') || engine.includes('web-tree-sitter'), '引擎路径指向 node_modules 的 web-tree-sitter')
  ...
  ok(engine2.includes('web-tree-sitter'), 'resourcesPath 有值但没行李 → 仍走 node_modules(不瞎信环境)')
...
console.log('── 打包态(resources/wasm/ 里引擎真在)→ 切行李箱 ──')
{
  ...
  ok(engineWasmPath({ resourcesPath: fake, cwd: CWD }) === join(wasmDir, 'tree-sitter.wasm'), '引擎路径切到 resources/wasm/')
  ok(grammarWasmDir({ resourcesPath: fake, cwd: CWD }) === wasmDir, '字典目录切到 resources/wasm/(平铺)')

  rmSync(join(wasmDir, 'tree-sitter.wasm'))
  ok(engineWasmPath({ resourcesPath: fake, cwd: CWD }).includes('web-tree-sitter'), '行李箱里引擎被拿走 → 回退 node_modules(认文件不认环境)')
...
console.log('── 行李清单对账:两家用到的语法字典,electron-builder.yml 的 filter 必须全带上 ──')
{
  ...
  ok(absentOnDisk.length === 0, `dev 家底 ${allFiles.size} 件字典真实在盘上${absentOnDisk.length ? `(缺:${absentOnDisk.join('、')})` : ''}`)
}
