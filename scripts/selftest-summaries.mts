...
import {
  ...
  rulePresetQuestions
} from '../src/shared/presetQuestions.ts'
...
function dir(
  name: string,
  children: ScanFileNode[] | ScanDirNode[] = [],
...
async function main(): Promise<void> {
  ...
  assert.ok(
    textOf(lock).includes('不要手动改') && lock.summary?.sticky === true,
    '锁定文件:风险句 + sticky 豁免降噪'
  )
  assert.ok(
    textOf(envFile).includes('不要外传') && envFile.summary?.sticky === true,
    '.env:风险句带"不要外传" + sticky'
  )
  ...
  for (const [node, keyword] of cases) {
    ...
    assert.ok(
      textOf(node).includes(keyword),
      `${node.name} 应提到「${keyword}」,实际:${textOf(node)}`
    )
  }
  ...
  annotateSummaries(
    dir('p', [entry, cssFile, kt, plainTs, docMd, photo, silentTxt, zipFile, unknownExt, noExt])
  )
  ...
  assert.ok(
    textOf(vendor).includes('第三方代码') && textOf(vendor).includes('不修改'),
    'vendor 说清只借不改'
  )
  assert.ok(textOf(tests).includes('测试代码'), 'tests → 测试代码')
  assert.ok(
    textOf(nm).includes('删了能重装') && nm.summary?.sticky === true,
    'node_modules 风险句(定稿文案)'
  )
  ...
  assert.ok(
    textOf(selftests).includes('7 个测试脚本'),
    `全自测的 scripts 要报数量,实际:${textOf(selftests)}`
  )
  ...
  // ── 6. 内容兜底:没名字线索时看装了啥 ──
  const mysteryCode = dir(
    'mystery',
    Array.from({ length: 5 }, (_, i) => file(`m${i}.ts`, TS))
  )
  ...
  assert.ok(
    textOf(mysteryCode).includes('5 个文件') && textOf(mysteryCode).includes('TypeScript'),
    '内容兜底要说数量和主语言'
  )
  assert.ok(
    textOf(notes).includes('全是文档') && textOf(notes).includes('2 份'),
    '全文档目录如实报份数'
  )
  ...
  const allDirsTagged = (n: ScanDirNode): boolean =>
    n.summary !== undefined &&
    n.children.every((c) => (c.type === 'directory' ? allDirsTagged(c) : true))
  ...
  assert.ok(
    textOf(locked).includes('系统保护目录'),
    `被锁的空目录要说系统保护,实际:${textOf(locked)}`
  )
  ...
  assert.ok(
    textOf(emptyConfig).includes('空的，还没有内容'),
    `探空的 config 要老实说空,实际:${textOf(emptyConfig)}`
  )

  const cutRoot = dir(
    'huge',
    Array.from({ length: 3 }, (_, i) => file(`f${i}.ts`, TS)),
    { truncated: true }
  )
  annotateSummaries(dir('proj', [cutRoot]))
  assert.ok(
    textOf(cutRoot).includes('至少 3 个文件'),
    `截断的家底要说「至少」,实际:${textOf(cutRoot)}`
  )

  const cutNotes = dir('notes', [file('a.md'), file('b.md'), file('c.md')], { truncated: true })
  annotateSummaries(dir('proj', [cutNotes]))
  assert.ok(
    textOf(cutNotes).includes('至少 3 份'),
    `截断的文档目录也要说「至少」,实际:${textOf(cutNotes)}`
  )
  ...
  assert.ok(
    stickyLock.summary?.sticky === true && textOf(stickyLock).includes('不要手动改'),
    '风险句保留 sticky 标记'
  )
  ...
  annotateSummaries(
    dir('proj', [scannerFile, gitFile, askHook, dataUtil, cssIndex, sumDir, graphDir])
  )
  assert.ok(textOf(scannerFile).includes('扫描器'), `scanner → 扫描器,实际:${textOf(scannerFile)}`)
  assert.ok(
    textOf(gitFile).includes('git') && textOf(gitFile).includes('状态'),
    `GitFileStatus → git状态,实际:${textOf(gitFile)}`
  )
  ...
  assert.ok(
    textOf(cssIndex).includes('样式') && !textOf(cssIndex).includes('索引'),
    'index.css 不给「索引」空话'
  )
  ...
  assert.ok(
    textOf(longDoc).includes('…') && textOf(longDoc).length <= 24,
    `超长标题掐头留省略号,实际:${textOf(longDoc)}`
  )
  ...
  assert.ok(
    entryHit.hits.some((h) => h.relPath === 'index.ts'),
    `入口要找到 index.ts,实际:${JSON.stringify(entryHit.hits)}`
  )
  const configHit = findCategory(catTree, '配置')
  assert.ok(
    configHit.hits.some((h) => h.relPath === 'config') && configHit.total >= 2,
    `配置要找到 config 目录和 app.json,实际:${JSON.stringify(configHit)}`
  )
  const testHit = findCategory(catTree, '测试')
  assert.ok(
    testHit.hits.some((h) => h.relPath === 'tests/a.test.ts'),
    `测试要找到考卷,实际:${JSON.stringify(testHit.hits)}`
  )
  const docHit = findCategory(catTree, '文档')
  assert.ok(
    docHit.hits.some((h) => h.relPath === 'README.md'),
    `文档要找到 README,实际:${JSON.stringify(docHit.hits)}`
  )
  ...
  assert.deepEqual(
    rulePresetQuestions({ name: 'a.test.ts', icon: 'test', text: '测试：验证代码对不对' }).slice(
      0,
      1
    ),
    ['它测的是哪个模块？'],
  ...
  assert.ok(
    rulePresetQuestions({ name: 'tsconfig.json', icon: 'config', text: '编译配置' }).includes(
      '动哪一项会影响什么？'
    ),
    '配置文件出配置专属题'
  )
  assert.ok(
    rulePresetQuestions({ name: '.env.local', icon: 'key', text: '配置开关' }).includes(
      '该提交它还是忽略它？'
    ),
    '密钥文件出安全专属题'
  )
  assert.ok(
    rulePresetQuestions({ name: 'README.md', icon: 'doc', text: '文档' }).includes(
      '信息过时了吗？'
    ),
    '文档出文档专属题'
  )
  assert.deepEqual(
    rulePresetQuestions({ name: 'zzz.xyz' }),
    DEFAULT_PRESET_QUESTIONS,
    '认不出的文件回万金油四问,永不空场'
  )
  ...
  console.log(
    '   三档词条(沉默/说明/风险+行动) · 模式规则 · 范畴词与诚实话 · 目录正脸 · scripts 报数 · 家底聚合 · 未展开占位 · 锁定目录 · 事实压绰号 · 残账至少 · 风险句标记 · 词根词典 · 类目直找 · 预设问题规则层 · 预览对话推荐问题'
  )
}
