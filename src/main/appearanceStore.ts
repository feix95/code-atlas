...
import { readJsonFile } from '../shared/jsonFile.ts'
...
function readStore(userDataDir: string): Appearance | null {
  const raw = readJsonFile(appearanceFilePath(userDataDir))
  // readJsonFile 的 null 是「没存过」的信号,不能喂给 sanitize(它会把 null 洗成默认外观)
  return raw === null ? null : sanitizeAppearance(raw)
}
