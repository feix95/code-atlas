...
/**
 * AI 解释状态机(纯钩子,不含组件):选中文件/文件夹后绝不自动请求,
 * AI 只在用户点了按钮或挑了问题后才跑。一 turns 记成一条 turn:请求中 → 完成/失败/取消(→ 重试)。
 * 这是「解释」通道:证据优先的单问单答。带历史的自由聊天在 useAiChat.ts,两条通道分家。
 * requestId + aiCancel 机制:每轮请求带随机 id,取消就掐;
